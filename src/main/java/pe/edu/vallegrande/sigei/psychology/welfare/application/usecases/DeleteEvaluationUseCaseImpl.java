...
import pe.edu.vallegrande.sigei.psychology.welfare.domain.ports.in.IDeleteEvaluationUseCase;
...
public class DeleteEvaluationUseCaseImpl implements IDeleteEvaluationUseCase {
    ...
    public Mono<PsychologicalEvaluation> execute(UUID id) {
        return repository.findById(id)
                ...
                .flatMap(evaluation -> {
                    evaluation.setStatus("INACTIVE");
                    evaluation.setUpdatedAt(LocalDateTime.now());
                    return repository.save(evaluation);
                })
                .doOnSuccess(deleted -> log.info("Evaluación desactivada (eliminado lógico): {}", id));
    }
}
